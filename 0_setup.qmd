...
-   Recommended: [Miniforge](https://github.com/conda-forge/miniforge#mambaforge){target="_blank"}
...
::: bashblock
```{bash install-aodp-in-conda-env-1, eval=FALSE, results='hide'}
...
```{bash install-aodp-in-conda-env-2, eval=FALSE, results='hide'}
...
```{r sessionInfo, eval = TRUE, echo = FALSE, message = FALSE, warning = FALSE, results = 'hide'}
...
::: bashblock
```{bash project-folder, eval=TRUE, results = 'hide'}
